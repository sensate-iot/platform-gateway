...
namespace SensateIoT.Platform.Network.GatewayAPI.Authorization
{
	public class BulkMeasurementAuthorizationService : AbstractAuthorizationHandler<JsonGatewayRequest<MeasurementPart>>, IBulkMeasurementAuthorizationService 
	{
		...
		private readonly ILogger<BulkMeasurementAuthorizationService> m_logger;
		private readonly IServiceProvider m_provider;

		public BulkMeasurementAuthorizationService(IServiceProvider provider, IHashAlgorithm algo, IRouterClient client, ILogger<BulkMeasurementAuthorizationService> logger)
		{
		...
		{
			List<JsonGatewayRequest<MeasurementPart>> messages;

			this.m_lock.Lock();

			try {
				var newList = new List<JsonGatewayRequest<MeasurementPart>>();
		...
		private async Task<int> WriteToRouterAsync(IEnumerable<JsonGatewayRequest<MeasurementPart>> measurements)
		{
		...
		private async Task SendBatchToRouterAsync(int start, int count, IList<Measurement> messages)
		{
			var data = new MeasurementData();

			for(var idx = start; idx < start + count; idx++) {
				var message = messages[idx];
				data.Measurements.Add(message);
			}
		...
		private async Task<IList<Measurement>> BuildMeasurementList(IEnumerable<JsonGatewayRequest<MeasurementPart>> messages)
		{
			Sensor sensor = null;
			...
			foreach(var message in messages) {
				try {
					...
					ConvertToContractMeasurement(message, sensor, data);
				} catch(Exception ex) {
		...
		private static void ConvertToContractMeasurement(JsonGatewayRequest<MeasurementPart> message, Sensor sensor, List<Measurement> data)
		{
		...
		protected override bool AuthorizeMessage(JsonGatewayRequest<MeasurementPart> message, Sensor sensor)
		{
